...
x. Using C and C++, Rcpp for peformance.
...
- Sharing: Make processes you have developed accessible to others by installing the package, not copying code or script files.
- Reusability: Packages will usually bundle functionality that can be used across many projects.
- Reproducibility: Packaging code with a build version for use in an [Reproducible Analytical Pipeline](https://analysisfunction.civilservice.gov.uk/support/reproducible-analytical-pipelines/).
- Clarity: A known system for structuring code repository, documenting processes, testing them.
- Understanding: Developing an R package gives builds knowledge of how R works.
...
- The roxygen comment is generates the help for the function.
- Need to add:
...
- See chapter 7 of [Building R Packages](https://r-pkgs.org/Code.html) for discuss on these things.
...
#' @importFrom magrittr %>%
```
- `use_pipe()` is a way to set a package so can use magrittr pipe `%>%` in every function 
